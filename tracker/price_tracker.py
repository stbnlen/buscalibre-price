...
class PriceTracker:
    ...
    def get_data(self):
        ...
        today_file = f"books/books_{datetime.now().strftime('%m-%d-%Y')}.csv"
        df = pd.DataFrame(
            [
    ...
    def compare_prices(self):
        yesterday_file = f"books/books_{(datetime.now() - timedelta(days=1)).strftime('%m-%d-%Y')}.csv"

        if os.path.exists(yesterday_file):
            yesterday_data = (
                pd.read_csv(yesterday_file).set_index("Title")["Price"].to_dict()
            )
            self.changes = [
                (
                    (
                    ...
                    if old_price is None
                        ...
                        if current_product.price > old_price
                        else (
                            (
                                title,
                                "Bajó",
                            ...
                            if current_product.price < old_price
                            else (title, "Sin cambios", 0, current_product.price)
                        )
                    )
                )
            ...
            print(
                f"No se encontró archivo para {yesterday_file}. No se pueden comparar precios."
            )
        ...
        if self.changes:
            pd.DataFrame(
                self.changes, columns=["Title", "Status", "Difference", "Current Price"]
            ).to_csv(
                f"price_changes/price_changes_{datetime.now().strftime('%m-%d-%Y')}.csv",
                index=False,
                encoding="utf-8",
            )
    ...
    def show_changes_window(self):
        ...
        tree = ttk.Treeview(
            window,
            columns=("Title", "Status", "Difference", "Current Price"),
            show="headings",
        )
